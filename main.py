...
def leaking_k(r,s,k):
    return (invert(int(s,16)+int(r,16),n)*(k-int(s,16)))%n

def oneuser_reusing_k(r1,s1,r2,s2):
    return ((int(s2,16)-int(s1,16))*invert(int(s1,16)-int(s2,16)+int(r1,16)-int(r2,16),n))%n

def differentuser_reusing_k(r1,s1,r2,s2,k):
    return ((k-int(s1,16))*invert(int(s1,16)+int(r1,16),n))%n,((k-int(s2,16))*invert(int(s2,16)+int(r2,16),n))%n
...
# sign=signature(m,Za,da)
#k = 0x6CB28D99385C175C94F94E934817663FC176D925DD72B727260DBAAE1FB2F96F
#print(hex(leaking_k(*sign,k)))
...
# m1=ENTLa+ida+'{:064X}'.format(a)+'{:064X}'.format(b)+'{:064X}'.format(Gx)+'{:064X}'.format(Gy)+'{:064X}'.format(Pa[0])+'{:064X}'.format(Pa[1])
# Za=hex(int(Hash(m1),16))[2:].upper()
# sign1=signature(m,Za,da)
# m2="test"
# m2=hex(int(binascii.b2a_hex(m.encode()).decode(),16)).upper()[2:]
# m1=ENTLa+ida+'{:064X}'.format(a)+'{:064X}'.format(b)+'{:064X}'.format(Gx)+'{:064X}'.format(Gy)+'{:064X}'.format(Pa[0])+'{:064X}'.format(Pa[1])
# Za=hex(int(Hash(m1),16))[2:].upper()
# sign2=signature(m2,Za,da)
# print(hex(oneuser_reusing_k(*sign1,*sign2)))

#da=0x128B2FA8BD433C6C068C8D803DFF79792A519A55171B1B650C23661D15897263
#db=0x985B2FB8BD433C6C068C8D803DFF79792A519A55171B1B650C23661D15897211
'--------------------------------different users reuse k------------------------------'
# k = 0x6CB28D99385C175C94F94E934817663FC176D925DD72B727260DBAAE1FB2F96F
# Pax,Pay=mul_add(Gx,Gy,da)
# Pa=(Pax,Pay)
# ma="message digest"
# ma=hex(int(binascii.b2a_hex(ma.encode()).decode(),16)).upper()[2:]
# IDa="ALICE123@YAHOO.COM"
# ida=hex(int(binascii.b2a_hex(IDa.encode()).decode(),16)).upper()[2:]
# ENTLa='{:04X}'.format(len(ida)*4)
# m1=ENTLa+ida+'{:064X}'.format(a)+'{:064X}'.format(b)+'{:064X}'.format(Gx)+'{:064X}'.format(Gy)+'{:064X}'.format(Pa[0])+'{:064X}'.format(Pa[1])
# Za=hex(int(Hash(m1),16))[2:].upper()
# sign1=signature(ma,Za,da)
#
# db=0x185B2FB8BD433C6C068C8D803DFF79792A519A55171B1B650C23661D15897211
# Pbx,Pby=mul_add(Gx,Gy,db)
# Pb=(Pbx,Pby)
# mb="test"
# mb=hex(int(binascii.b2a_hex(mb.encode()).decode(),16)).upper()[2:]
# IDb="BOB12345@YAHOO.COM"
# idb=hex(int(binascii.b2a_hex(IDb.encode()).decode(),16)).upper()[2:]
# ENTLb='{:04X}'.format(len(idb)*4)
# m1=ENTLb+idb+'{:064X}'.format(a)+'{:064X}'.format(b)+'{:064X}'.format(Gx)+'{:064X}'.format(Gy)+'{:064X}'.format(Pb[0])+'{:064X}'.format(Pb[1])
# Zb=hex(int(Hash(m1),16))[2:].upper()
# sign2=signature(mb,Zb,db)
# t1,t2=differentuser_reusing_k(*sign1,*sign2,k)
# print(hex(t1),hex(t2))
